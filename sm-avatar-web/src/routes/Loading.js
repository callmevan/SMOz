...
import { createScene, setAvatar, setUserID } from '../store/sm';
...
function Loading({
  ...
  useEffect(() => {
    if (isValid(userID)) dispatch(setUserID(userID));
    // console.log('userID from effect', userID);
  }, [userID]);
  
  const { avatar } = useParams();
  useEffect(() => {
    if (isValid(avatar)) dispatch(setAvatar(avatar));
    // console.log('avatar from effect', avatar);  
  }, [avatar]);
  ...
  useEffect(() => {
    if (skip === true && connected === true) {
      ...
      if (!userID && !avatar) {
        ...
        localStorage.setItem('reDirectUrl', `/video/${avatar}?userID=${userID}`);
        history.push(`/video/${avatar}?userID=${userID}`);
      }
    }
  }, [connected, skip]);
