...
const _log = (...msg) => {
  console.log('store.sm.index|', ...msg);
};
...
_log('TOKEN_ISSUER', TOKEN_ISSUER);
_log('ORCHESTRATION_URL', ORCHESTRATION_URL);
...
const createSessionMap = createAsyncThunk('sm/createSessionMap', async (args, thunk) => {
  _log('createSessionMap');

  if (!scene) {
    return console.error('cannot create session map, scene not initiated!');
  }
  
  const { sessionID, avatar, userID } = thunk.getState().sm;
  _log('creating session map', sessionID, avatar, userID);
...
export const disconnect = createAsyncThunk('sm/disconnect', async (args, thunk) => {
  ...
  if (scene && (loading === false || loading === undefined)) scene.disconnect();
...
export const createScene = createAsyncThunk('sm/createScene', async (_, thunk) => {
  ...
  try {
    ...
    if (AUTH_MODE === 1) {
      ...
      const { avatar } = thunk.getState().sm;
      console.log(`fetching token from token server for ${avatar} ...`);
      const [tokenErr, tokenRes] = await to(fetch(TOKEN_ISSUER, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ avatar }) }));
      console.log('fetching token complete', tokenRes);
    ...
    const userText = 'test-custom-user-text';
    const connectP = scene.connect(url, userText, jwt, retryOptions);
    const [err, sessionID] = await to(connectP);
    if (err) {
      switch (err.name) {
        case 'notSupported':
        case 'noUserMedia': {
    ...
    // TODO: the following lines generate error in the console 
    // "noConnection: Connection has not been established"
    // Fixed by moving the code after connection established
...
const smSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createScene.fulfilled, (state) => ({
        ...state,
        loading: false,
        connected: true,
        startedAt: Date.now(),
      ...
      .addCase(createScene.rejected, (state, { error }) => {
        try {
          scene.disconnect();
        } catch {
        ...
        return {
          ...
          loading: false,
          connected: false,
          error: { msg: `createScene.rejected: ${error.message}` },
        };
      });
  },
});
